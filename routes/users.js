...
router.post('/signup', async (req, res) => {
    try {
        ...
        const newUser = await userData.addUser(nickname, email, false);
...
router.post('/admin/signup', async (req, res) => {
    try {
        ...
        if (!userInfo) {
            res.status(400).json({
                error: 'You must provide data to create an admin user',
            });
            return;
        }
        ...
        const newUser = await userData.addUser(nickname, email, true);
        res.status(newUser.status).json(newUser.result);
    } catch (error) {
        if (error.response) {
            res.status(error.response.status).json({
